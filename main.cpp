...
int main(int argc, char* argv[]) {
    ...
    while(true){    
        ...
        for (std::size_t i = 0; i < transformed_points.size(); ++i) {
            ...
            double rx = pointRotations[i].first;
            double ry = pointRotations[i].second;
            char readyNext = '0';
            
            //go dispenser
            rtde_control.moveL_FK({-1.187, -1.257, -1.92, -1.553, 1.623, 0},0.2,0.2);
            ...
            rtde_control.moveJ_IK({-0.287, -0.217, 0.054, 2.699, -1.565, -2.521},0.5);
            rtde_control.moveL({-0.334, -0.149, 0.026, 2.584, -1.685, -2.509},0.1);
            
            rtde_control.moveL({-0.345, -0.096, 0.027, 2.584, -1.685, -2.509},0.05);
            rtde_control.moveL({-0.340, -0.096, 0.027, 2.584, -1.685, -2.509},0.005);
            
            std::cout << "Grippuh Cluh" << std::endl;
            send_and_receive(pico_serial, "Pico", '1');

            rtde_control.moveL({-0.340, -0.096, 0.04, 2.584, -1.685, -2.509},0.005);
            rtde_control.moveL({-0.334, -0.149, 0.04, 2.584, -1.685, -2.509},0.05);
            
            rtde_control.moveJ_IK({-0.203, -0.312, 0.201, 2.46, -1.955, -1.492},0.3);
            rtde_control.moveJ({-1.187, -1.257, -1.92, -1.553, 1.623, 0},0.5);

            rtde_control.moveJ_IK({x, y, operationHeight, rx, ry, -0},0.5);
            rtde_control.moveL({x, y, placeHeight, rx, ry, -0},0.1,0.1);

            std::cout << "Grippuh Opuh" << std::endl;
            send_and_receive(pico_serial, "Pico", '0');

            rtde_control.moveL({x, y, operationHeight, rx, ry, -0},0.1,0.1);
            /*
            while (readyNext == '0') {
        ...
        if (knock == '1') {
            ...
            double rx = pointRotations[0].first;
            double ry = pointRotations[0].second;

            rtde_control.moveJ_IK({x, y, operationHeight, rx, ry, -0},0.2);
            rtde_control.moveL({x, y, placeHeight, rx, ry, -0},0.1,0.1);
