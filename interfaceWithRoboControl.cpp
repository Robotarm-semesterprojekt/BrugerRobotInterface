...
int main(int argc, char* argv[]) {
    ...
    const double operationHeight = 0.2;
    const double placeHeight  = 0.1;
    ...
    while(true){    
        ...
        // Output the number of points and the list of points, both in interface space and real space
        ...
        for (std::size_t i = 0; i < transformed_points.size(); ++i) {
            ...
            rtde_control.moveJ_IK({-0.4, -0.076, operationHeight, 0.081, -1.949, -1.775});
            rtde_control.moveL({-0.4, -0.076, 0.045, 0.081, -1.949, -1.775},0.1,0.2);
            rtde_control.moveL({-0.4, -0.076, operationHeight, 0.081, -1.949, -1.775},0.1,0.2);

            rtde_control.moveJ_IK({x, y, operationHeight, 0.8, -3, -0});
            rtde_control.moveL({x, y, placeHeight, 0.8, -3, -0},0.1,0.2);
            rtde_control.moveL({x, y, operationHeight, 0.8, -3, -0},0.1,0.2);
        }
    }
